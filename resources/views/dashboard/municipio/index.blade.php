...
<title>VIAJETUR | MUNICIPIOS</title>
        ...
        <div class="container">
            <div class="content-header mb-4">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h4 class="m-0"><i class="fas fa-map-marker-alt"></i> MUNICIPIOS DISPONIBLES</h4>
                        </div>
                        @if (Auth()->user()->role->id != 4 && Auth()->user()->role->administrar_municipios == 1 && Auth()->user()->role->agregar == 1)
                        <div class="col-sm-6">
                            <div class="float-sm-right">
                                <a href="{{url('municipio/create')}}" class="btn btn-success">AGREGAR</a>
                            </div>
                        </div>
                        @endif
                    </div>
                ...
                <table class="table table-light table-hover text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class=""><p>{{$municipio->nombre}}</p></td>
                            @if (Auth()->user()->role->id != 4 && (Auth()->user()->role->editar == 1 || Auth()->user()->role->borrar == 1) )    
                            <td class="">
                                <form class="container-fluid" action="{{url('municipio/'.$municipio->id.'/')}}" method="post">
                                    @if (Auth()->user()->role->id != 4 && Auth()->user()->role->administrar_municipios == 1 && Auth()->user()->role->editar == 1)
                                    <a href="{{url('/municipio/'.$municipio->id.'/edit')}}" class="btn btn-success"><i class="fas fa-pencil-alt"></i></a>    
                                    @endif
                                    @if (Auth()->user()->role->id != 4 && Auth()->user()->role->administrar_municipios == 1 && Auth()->user()->role->borrar == 1)    
                                    @csrf
                                    {{method_field('DELETE')}}
                                    <button type="submit"  class="btn btn-danger" onclick="return confirm('¿Quieres borrar {{$municipio->nombre}} de la lista de municipio?')" value="{{$municipio->id}}"><i class="fas fa-trash"></i></button>
                                    @endif
